/* @file TagIntegratedValidate.hpp
 * this was the first version of TagIntegrated.hpp but has additional testing objects methods
 * I didn't want these test objects and inefficiencies in the actual model as it adds unneccessary execution cost
 * The downside is we have to repeat the source code. Any fundamental change in TagIntegrated should be added to this so
 * the unit test integrity and coverage is mainained
 * We build basic tests using this model. And then compare more complex tests between this model and the
 * production model
 */

#ifndef TagIntegratedValidate_hpp
#define TagIntegratedValidate_hpp
...
//
template<class Type>
Type TagIntegratedValidate(objective_function<Type>* obj) {
  ...
  // this will effect the expected size of the parameter 'trans_rec_dev', if global_rec_devs = 1. then trans_rec_dev.size() = n_years + n_ages + 1 else n_regions * (n_years + n_ages + 1). with the first (n_years + n_ages + 1) corresponding to region 1 and so in block
  ...
  PARAMETER_ARRAY(transformed_movement_pars);               // transformed parameters for movmenet (consider both simplex and logistic? or what ever it is). dimension:  (n_regions - 1) x n_regions x n_movement_time_blocks
  ...
  // deal with movement
  array<Type> movement_matrix(n_regions, n_regions, n_movement_time_blocks);                  // n_regions x n_regions x n_movement_time_blocks. Rows sum = 1 (aka source)
    ...
    for(region_ndx = 0; region_ndx < n_regions; ++region_ndx) {
      ...
      if(tag_year_counter > 0) { // there has to be at least one tag cohort in the partition before we waste resources
        ...
        if(tag_likelihood == 2) {
          for(tag_ndx = 0; tag_ndx < n_years_to_retain_tagged_cohorts_for; ++tag_ndx) {
            ...
            for(release_region_ndx = 0; release_region_ndx < n_regions; ++release_region_ndx) {
              if(tag_recovery_indicator(release_year_ndx, release_region_ndx) == 1) {
                ...
                //std::cerr << "year_ndx " << year_ndx << " tag_ndx "<<  tag_ndx << " release_year_ndx " << release_year_ndx << " recovery region = " <<region_ndx <<  " release_region_ndx " << release_region_ndx << " Calculate tag prediction tag-recovery event " << tag_recovery_event_ndx << " pred " << predicted_tags << "\n";
          ...
          // Poisson or Negative Binomial tag likelihood
