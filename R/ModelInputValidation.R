...
validate_input_data_and_parameters = function(data, parameters) {
  ...
  if(data$do_projection == 1) {
    ...
    if(data$n_projections_years != 0)
      stop("data$n_projections_years must be equal to 0 when data$do_projection == 0")
  }
  ...
  check = check_length(data$tag_recovery_indicator, n_years)
  if(!check$result)
    return(paste0("tag_recovery_indicator: ", check$message))

  n_tag_recoveries = sum(data$tag_recovery_indicator)
  if(n_tag_recoveries > 0) {
    ## tag_recovery_indicator_by_release_event_and_recovery_region
    check = check_dim(data$tag_recovery_indicator_by_release_event_and_recovery_region, c(n_regions * (data$n_years_to_retain_tagged_cohorts_for + 1), n_regions, n_tag_recoveries))
    if(!check$result)
