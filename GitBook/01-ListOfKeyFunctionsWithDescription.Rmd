...
- `find_apportioned_fref` this will find a single F assuming future deterministic recruitment that achieves some specified target global percent \(B_{0}\). Users must specify how much of that sinlge F is allocated to each fishery in each region.
- `find_regional_fref` this will attempt to find an F for each region and fishery assuming future deterministic (mean) recruitment that achieves some specified target percent \(B_{0}\) in each region. Worth checking the output for this function when there is movement among areas. This connectivity assumption can result in difficult optimisation.
