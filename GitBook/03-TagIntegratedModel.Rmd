...
** An important input is whether movement occurs to new recruits, specified by the input `do_recruits_move`. If `do_recruits_move == 0`  i.e., they don't get movement applied then recruitment is applied to the partition after the movement process. In which case movement is applied with zeros for the recruited age in the partition. Recruitment in year \(y\) wont contribute to any model quantities in between \(y\) and \(y+1\). All model quantities are derived based on the partition at the start of the year \(y\), where as recruitment is added during the year and so does not enter model quantities until recruited age + 1 in the following year.
