...
\]
where, \(N^k_{a,r,y,s}\) are the number of tagged fish from tag-release event \(k\), and \(\delta_y\) is the reporting rate in year \(y\). We assume half the years natural mortality has taken place before deriving expected tag-recoveries from the fixed gear fishery. The current assessment pools observed and fitted values across age and sex,
\[
\widehat{N}^k_{r,y} = \sum_a \sum_s \widehat{N}^k_{a,r,y,s}
\]
...
\[
{N}^k_{r,y} \sim \mathcal{NB}(\widehat{N}^k_{r,y}, \hat{\phi})
\]
