...
\[
SSB_{y,r} \sum\limits_a N_{a,s = 2, r, y} S^{mat}_{a,y} \bar{w}_{a,s = 2, y}
\]

where, \(s = 2\) denotes the female sex, \(S^{mat}_{a,y}\) is the maturity at age and \(\bar{w}_{a,s = 2, y}\) is female mean weight at age
...
Technically, this derivation is only correct when `data$srv_dom_ll_bio_comp_likelihood == 1` however it will be approximately close for the other likelihood. Assuming a specific element contributes to the likelihood in the following manor,

\[
-ll = \sum_i log (\sigma_i) + 0.5\sum_i \bigg(\frac{log(O_i) - log(qE_i) + 0.5\sigma_i^2}{\sigma_i} \bigg)^2 \ ,
\]

then, if differentiate the negative log-likelihood with respect to \(q\) 
\[
\frac{\partial }{\partial q}(-ll) = \frac{-1}{q} \sum_i\bigg( \frac{log(O_i/E_i) - log(q) + 0.5\sigma_i^2}{\sigma_i^2}\bigg)
\]

we can solve for when the derivative is zero (MLE estimate) and obtain the following result
