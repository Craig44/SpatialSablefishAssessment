...
You can also apply/estimate additional fishing mortality during the initial equilibrium calculation denoted by \(F^{init}\). If `F_method = 0` then 
\[
F^{init} = F^{LL} p_{hist}
\]
where, \(F^{LL}\) is the average annual fishing mortality for the fixed gear (`ln_fixed_F_avg`) and \(p_{hist}\) is an input value that specifies the proportion of average longline fishing mortality to apply during initialisation. If `F_method = 1` then 
\[
F^{init} =  \widehat{F}^{init} p_{hist}
\]
