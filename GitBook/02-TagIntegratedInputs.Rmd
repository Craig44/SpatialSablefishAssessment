# Descriptions of data and parameter objects expected by the `TagIntegrated` model {#TagIntegrated}

Users need to population a `data` and `parameters` named list object that is then passed to TMBs `MakeADFun`. These sections outline the elements and corresponding dimensions of both `data` and `parameters`.
...
- `n_init_rec_devs` number of initial n_age_deviations to multiply against the initial numbers at age to have un equilbrium starting age-structure.
...
- `fixed_sel_type` vector of integers specifying the selectivity type in each time-block.
...
- `trwl_sel_type` vector of integers specifying the selectivity type in each time-block.
...
- `srv_dom_ll_sel_type` vector of integers specifying the selectivity type in each time-block.
